<template>
    <div class="app-container">
      ...
      >
        ...
        <el-table-column label="表单token" prop="formToken" align="center" width="100">
          <template slot-scope="{row}">
            <span>{{ row.formToken }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="公众号发布状态" prop="pubMenuIsShow" align="center" width="110">
          <template slot-scope="{row}">
            ...
            <span v-if="row.pubMenuIsShow == 0">未发布</span>
          </template>
        </el-table-column> -->
        ...
        <el-table-column label="操作" align="center" width="300" class-name="small-padding fixed-width">
          <template slot-scope="{row,$index}">
            ...
            <el-button v-if="row.pubMenuIsShow == 1" type="danger" size="mini" @click="qnNotPubMenuIsShow(row)">
                公众号撤回
            </el-button> -->
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        qnList(this.listQuery).then(response => {
      ...
      fileDown(row){
        axios({
          ...
          responseType: 'blob' // 表明返回服务器返回的数据类型
            ...
            aLink.href = url;
            aLink.setAttribute("download", row.title); // 下载的文件
      ...
      qnMiniIsShow(row, index) {
      this.$confirm('确认发布?', '提示', {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
        qnMiniIsShow(row.id,).then((res) => {
          if (res.code == 20000) {
            this.$notify({
              title: 'Success',
              message: '发布成功',
    ...
    qnNotMiniIsShow(row, index) {
      this.$confirm('确认撤回?', '提示', {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
        qnNotMiniIsShow(row.id,).then((res) => {
          if (res.code == 20000) {
            this.$notify({
              title: 'Success',
              message: '撤回成功',
    ...
    qnPubMenuIsShow(row, index) {
      this.$confirm('确认发布?', '提示', {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
        qnPubMenuIsShow(row.id,).then((res) => {
          if (res.code == 20000) {
            this.$notify({
              title: 'Success',
              message: '发布成功',
    ...
    qnNotPubMenuIsShow(row, index) {
      this.$confirm('确认撤回?', '提示', {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
        qnNotPubMenuIsShow(row.id,).then((res) => {
          if (res.code == 20000) {
            this.$notify({
              title: 'Success',
              message: '撤回成功',
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            //   return
            // }
            //  if(this.temp.title != null && this.temp.title.length > 8 ){
            //   this.$notify({
            //     message: '标题字数不能大于8个！',
            //     type: 'error',
            //     duration: 2000          
